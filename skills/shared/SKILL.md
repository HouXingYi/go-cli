---
name: {{AppName}}
description: Patterns for invoking {{AppName}} from agents. Covers auth, module defaults, api discovery, http proxy calls, and error recovery.
---
...
`{{AppName}}` 没有交互式提示。在非 TTY 环境（Agent shell、管道）中：
...
Hint: run {{AppName}} config module set <name> or pass --module.
```
...
6. `{{AppName}} http <method> <path> [--query ...] [--body ...]` — 调用 API。
...
3. `{{AppName}} config module set` 持久化在 `state.yaml` 中的值

若均未设置，`http` 报错 `module is required`。

```bash
{{AppName}} config module set ziniao
{{AppName}} config module get
{{AppName}} http GET /api/user/list --module erp   # 临时覆盖
```
...
```bash
{{AppName}} api                              # 一级大模块
{{AppName}} api <module>                     # 二级业务模块
{{AppName}} api <module> <business>          # API 摘要（method、url）
{{AppName}} api <module> <business> --full   # 该业务下全部 API 完整文档
{{AppName}} api <module> <business> <api>    # 单个 API 文档（参数、响应）
```
...
## HTTP 调用（`{{AppName}} http`）

```bash
{{AppName}} http <method> <path> [--module <name>] [--query '<json-object>'] [--body '<json>']
```
...
```bash
{{AppName}} http GET /api/user/list --query '{"page":1,"pageSize":20}'
{{AppName}} http POST /api/user/create --body '{"name":"test"}'
```
...
本 Skill 仅说明**如何驱动 {{AppName}}**。业务工作流（账号、员工、设备、访问策略等）优先查阅对应的 `ziniao-*` 业务 Skill。调用 `http` 前，务必用 `{{AppName}} api <module> <business> <api>` 确认参数结构。
